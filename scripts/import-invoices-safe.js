...
function executeSafeSql(sql) {
  try {
    // Tạo file SQL tạm thời
    const tempFile = path.join(__dirname, `temp_sql_${Date.now()}.sql`);
    fs.writeFileSync(tempFile, sql);

    // Sao chép file vào container
    execSync(`docker cp ${tempFile} supabase-db:/tmp/temp_sql.sql`);

    // Thực thi SQL từ file trong container
    const result = execSync(`docker exec -i supabase-db psql -U postgres -d postgres -f /tmp/temp_sql.sql -t -A`).toString();
    ...
    try {
      ...
      if (orderNo) {
        try {
          ...
          const result = executeSafeSql(sqlCommand);
...
// Hàm kiểm tra hóa đơn tồn tại
async function checkInvoiceExists(invoiceId) {
  try {
    const sqlCommand = `
    ...
    console.error(`Error checking if invoice ${invoiceId} exists:`, error.message);
    return false;
  }
}
...
async function importInvoiceDetails(invoiceRows, invoiceId, unitMapping, pricingTypeMapping, logStream) {
  ...
  if (!invoiceExists) {
    logStream.write(`Cannot import details: Invoice ${invoiceId} does not exist in the database\n`);
    return;
  }
  ...
  for (const row of invoiceRows) {
    try {
      ...
      if (descriptionKey && row[descriptionKey]) {
        ...
        let rawDescription = row[descriptionKey].substring(0, 500);

        // Thay thế hoàn toàn các ký tự có thể gây vấn đề
        rawDescription = rawDescription.replace(/'/g, " ");
        rawDescription = rawDescription.replace(/"/g, " ");
        rawDescription = rawDescription.replace(/\\/g, " ");
        rawDescription = rawDescription.replace(/\n/g, " ");
        rawDescription = rawDescription.replace(/\r/g, " ");
        rawDescription = rawDescription.replace(/\t/g, " ");

        // Loại bỏ các ký tự đặc biệt khác
        safeDescription = rawDescription.replace(/[^\w\s.,;:()[\]{}]/g, " ");
      }
      ...
      const insertCommand = `
        INSERT INTO invoice_details (
          id, invoice_id, description, quantity, unit_id, is_fixed_price, fixed_price,
...
async function main() {
  try {
    ...
    if (!csvFilePath || !clientMappingPath) {
      console.error('Usage: node import-invoices-safe.js <invoices-csv-path> <client-mapping-csv-path>');
      process.exit(1);
    }
