-- Script để import toàn bộ dữ liệu từ file Data AG.csv vào database
-- ID của team Agri
\set agri_team_id '26b6ad02-03c0-45ac-9b65-3caeac723829'

-- 1. Tạo bảng tạm thời để lưu dữ liệu từ CSV
DROP TABLE IF EXISTS temp_agri_data;
CREATE TEMP TABLE temp_agri_data (
    ...
    inspection_date_started TEXT,
    inspection_date_completed TEXT
);

-- 2. Import dữ liệu từ CSV vào bảng tạm thời
\COPY temp_agri_data FROM 'scripts/Data AG.csv' WITH (FORMAT csv, HEADER true, ENCODING 'UTF8');
...
-- 5. Cập nhật team_ids cho clients đã tồn tại
UPDATE clients c
SET team_ids = CASE
    WHEN c.team_ids IS NULL THEN ARRAY[:'agri_team_id'::UUID]
    WHEN :'agri_team_id'::UUID = ANY(c.team_ids) THEN c.team_ids
    ELSE c.team_ids || ARRAY[:'agri_team_id'::UUID]
END
FROM (
    SELECT DISTINCT client_name FROM temp_agri_data WHERE client_name IS NOT NULL AND client_name != ''
) t
...
SELECT DISTINCT ON (t.order_number)
    ...
    t.inspection_place,
    CASE 
        WHEN t.inspection_date_started ~ '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}$' THEN 
            TO_DATE(t.inspection_date_started, 'DD/MM/YYYY')
        ELSE 
            NULL
    END,
    CASE 
        WHEN t.inspection_date_completed ~ '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}$' THEN 
            TO_DATE(t.inspection_date_completed, 'DD/MM/YYYY')
        ELSE 
            NULL
    END,
...
-- 14. Xóa bảng tạm thời
DROP TABLE temp_agri_data;

-- 15. Hiển thị số lượng bản ghi đã import
SELECT 'clients' AS table_name, COUNT(*) AS record_count FROM clients WHERE :'agri_team_id'::UUID = ANY(team_ids)
UNION ALL
SELECT 'contacts' AS table_name, COUNT(*) AS record_count FROM contacts WHERE client_id IN (SELECT id FROM clients WHERE :'agri_team_id'::UUID = ANY(team_ids))
UNION ALL
...
UNION ALL
SELECT 'orders' AS table_name, COUNT(*) AS record_count FROM orders WHERE team_id = :'agri_team_id'::UUID
UNION ALL
SELECT 'order_items' AS table_name, COUNT(*) AS record_count FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE team_id = :'agri_team_id'::UUID);
