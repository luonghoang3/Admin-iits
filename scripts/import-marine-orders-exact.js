  ...
  data.forEach(row => {
    if (row.client_name && !uniqueClients.has(row.client_name)) {
      uniqueClients.set(row.client_name, {
        ...
        team_ids: ['e7697f59-05cc-434e-b679-fde4c53b7d7c'] // Marine team ID
      });
    }
  });
  ...
  for (const [clientName, clientData] of uniqueClients.entries()) {
    try {
      ...
      if (existingClients && existingClients.length > 0) {
        ...
        if (existingClients[0].team_ids && !existingClients[0].team_ids.includes('e7697f59-05cc-434e-b679-fde4c53b7d7c')) {
          const updatedTeamIds = [...existingClients[0].team_ids, 'e7697f59-05cc-434e-b679-fde4c53b7d7c'];
...
async function main() {
  try {
    ...
    const filePath = process.argv[2] || path.resolve(__dirname, 'Data MR.csv');
