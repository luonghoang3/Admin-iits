...
function findBestMatchingClient(oldClientName, clients, threshold = 0.5) {
...
async function main() {
  try {
    ...
    if (!csvFilePath) {
      console.error('Usage: node suggest-client-mapping.js <unique-clients-csv-path>');
      process.exit(1);
    }
    ...
    console.error('Error during suggestion process:', error);
    process.exit(1);
  }
}
