...
async function importOrders(data) {
  ...
  for (const [orderNumber, orderRows] of orderGroups.entries()) {
    try {
      ...
      let department = 'consumer_goods'; // Default based on the file name
      
      if (firstRow.department) {
        const deptLower = firstRow.department.toLowerCase();
        if (deptLower.includes('marine')) {
          department = 'marine';
        } else if (deptLower.includes('agri')) {
          department = 'agri';
        }
      }
      ...
      const orderData = {
        ...
        bill_of_lading_date: formatDate(firstRow.bill_of_lading_date) || null,
