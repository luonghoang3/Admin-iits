...
// Function to create default commodity and unit
async function createDefaultEntities() {
  console.log('Creating default commodity and unit for missing data...');

  try {
    // Create default commodity if it doesn't exist
    const { data: existingCommodities, error: searchError } = await supabase
      .from('commodities_new')
      .select('id, name')
      .eq('name', 'Unknown Commodity')
      .limit(1);

    if (searchError) {
      console.error('Error searching for default commodity:', searchError);
    } else {
      if (existingCommodities && existingCommodities.length > 0) {
        defaultCommodityId = existingCommodities[0].id;
        console.log(`Default commodity already exists with ID: ${defaultCommodityId}`);
      } else {
        const { data: newCommodity, error: insertError } = await supabase
          .from('commodities_new')
          .insert([{
            name: 'Unknown Commodity',
            description: 'Default commodity for missing data'
          }])
          .select();

        if (insertError) {
          console.error('Error creating default commodity:', insertError);
        } else {
          defaultCommodityId = newCommodity[0].id;
          console.log(`Created default commodity with ID: ${defaultCommodityId}`);
        }
      }
    }

    // Create default unit if it doesn't exist
    const { data: existingUnits, error: unitSearchError } = await supabase
      .from('units')
      .select('id, name')
      .eq('name', 'Unknown Unit')
      .limit(1);

    if (unitSearchError) {
      console.error('Error searching for default unit:', unitSearchError);
    } else {
      if (existingUnits && existingUnits.length > 0) {
        defaultUnitId = existingUnits[0].id;
        console.log(`Default unit already exists with ID: ${defaultUnitId}`);
      } else {
        const { data: newUnit, error: unitInsertError } = await supabase
          .from('units')
          .insert([{
            name: 'Unknown Unit',
            description: 'Default unit for missing data'
          }])
          .select();

        if (unitInsertError) {
          console.error('Error creating default unit:', unitInsertError);
        } else {
          defaultUnitId = newUnit[0].id;
          console.log(`Created default unit with ID: ${defaultUnitId}`);
        }
      }
    }

    return defaultCommodityId && defaultUnitId;
  } catch (error) {
    console.error('Error in createDefaultEntities:', error);
    return false;
  }
}
...
async function importOrders(data) {
  ...
  for (const [orderNumber, orderRows] of orderGroups.entries()) {
    try {
      ...
      console.log(`- Client: ${firstRow.client_Name} (${clientId})`);
...
// Modified function to import order items with fallback for missing data
async function importOrderItems(orderRows, orderId) {
  ...
  for (const row of orderRows) {
    try {
      ...
      // Try to get commodity ID, use default if not found
      if (row.commodity_name && commodityMap.has(row.commodity_name)) {
        commodityId = commodityMap.get(row.commodity_name);
      } else if (row.commodity_name) {
        // Create new commodity if it doesn't exist
        console.log(`Creating new commodity: "${row.commodity_name}"`);
        try {
          const { data: newCommodity, error: insertError } = await supabase
            .from('commodities_new')
            .insert([{
              name: row.commodity_name,
              description: row.item_description || row.commodity_name
            }])
            .select();
          ...
          console.error(`Error processing commodity ${row.commodity_name}:`, error);
          commodityId = defaultCommodityId;
        }
      } else {
        commodityId = defaultCommodityId;
      ...
      // Try to get unit ID, use default if not found
      if (row.unit_name && unitMap.has(row.unit_name)) {
        unitId = unitMap.get(row.unit_name);
      } else if (row.unit_name) {
        // Create new unit if it doesn't exist
        console.log(`Creating new unit: "${row.unit_name}"`);
        try {
          const { data: newUnit, error: insertError } = await supabase
            .from('units')
            .insert([{
          ...
          console.error(`Error processing unit ${row.unit_name}:`, error);
          unitId = defaultUnitId;
        }
      } else {
        unitId = defaultUnitId;
      ...
      // Skip if we couldn't determine commodity or unit
      if (!commodityId || !unitId) {
        console.warn(`Skipping order item: Missing required commodity or unit IDs.`);
        continue;
      }
      ...
      const orderItemData = {
        ...
        quantity: quantity,
        unit_id: unitId,
        commodity_description: row.item_description || row.commodity_name || 'No description'
      };
      ...
      if (existingItems && existingItems.length > 0) {
        ...
        if (updateError) {
          ...
          console.log(`Updated order item for commodity "${row.commodity_name || 'Unknown'}"`);
        }
      } else {
        ...
        if (insertError) {
          ...
          console.log(`Created order item for commodity "${row.commodity_name || 'Unknown'}"`);
        }
      }
    } catch (error) {
...
// Function to create a default order item for orders without items
async function createDefaultOrderItem(orderId) {
  try {
    ...
    if (searchError) {
      console.error(`Error searching for order items:`, searchError);
      return false;
    }

    // Only create default item if no items exist
    if (!existingItems || existingItems.length === 0) {
      const orderItemData = {
        order_id: orderId,
        commodity_id: defaultCommodityId,
        quantity: 1,
        unit_id: defaultUnitId,
        commodity_description: 'Default item for order without details'
      };
      ...
      if (insertError) {
        console.error(`Error creating default order item:`, insertError);
        return false;
      } else {
        console.log(`Created default order item for order ID: ${orderId}`);
        return true;
      }
    }
    ...
    console.error(`Error in createDefaultOrderItem:`, error);
    return false;
  }
}

// Function to ensure all orders have at least one order item
async function ensureAllOrdersHaveItems() {
  console.log('Ensuring all orders have at least one order item...');
  ...
  try {
    ...
    const { data: ordersWithoutItems, error } = await supabase
      .from('orders')
      .select('id, order_number')
      .not('id', 'in', `(SELECT DISTINCT order_id FROM order_items)`);

    if (error) {
      console.error('Error fetching orders without items:', error);
      return false;
    }

    if (ordersWithoutItems && ordersWithoutItems.length > 0) {
      console.log(`Found ${ordersWithoutItems.length} orders without items. Creating default items...`);

      let successCount = 0;
      for (const order of ordersWithoutItems) {
        const success = await createDefaultOrderItem(order.id);
        if (success) {
          successCount++;
        }
        ...
        if (successCount % 100 === 0) {
          console.log(`Progress: ${successCount}/${ordersWithoutItems.length} orders processed`);
        }
      }

      console.log(`Created default items for ${successCount}/${ordersWithoutItems.length} orders`);
      return true;
    } else {
    ...
    console.error('Error in ensureAllOrdersHaveItems:', error);
    return false;
  }
}
...
async function main() {
  try {
    ...
    // Create default entities for missing data
    ...
    // Ensure all orders have at least one order item
    await ensureAllOrdersHaveItems();
