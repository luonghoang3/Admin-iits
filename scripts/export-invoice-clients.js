...
async function main() {
  try {
    ...
    if (!csvFilePath) {
      console.error('Usage: node export-invoice-clients.js <invoices-csv-path>');
      process.exit(1);
    }
    ...
    const outputPath = path.join(path.dirname(csvFilePath), 'unique-invoice-clients.csv');
